...
def generate_a6t_command_for_index(data: Dict[str, list], idx: int, dt_ms: int) -> str:
    """生成某一时刻对应的 @A6T 指令"""
    # 假设控制器协议里：最后一个参数单位是 10ms
    device_time_unit_ms = 10
    device_dt = max(1, dt_ms // device_time_unit_ms)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # 全局参数（真实世界时间体系）
        self.base_z = 40.0
        self.dt_ms = 10               # 每步发送间隔(ms)，也是仿真采样间隔
        self.k_roll = 1.0
        self.k_pitch = 1.0
        self.k_yaw = 1.0
    ...
    def on_stop_clicked(self):
        ...
        # 当前时间索引：play_index 指向“下一帧”，所以当前帧为 play_index-1
        ...
        self.stop_start_time = data["t"][idx]   # 停止瞬间的真实时间(秒)

        # 插补步数，例如 stop_duration_ms = 2000, dt_ms = 10 → 2000/10 = 200 步
